...
export function MyFormDialogLoad({ open, setOpen, land }: MyFormDialogProps) {
  ...
  const form = useForm<z.infer<typeof landRegistrySchema>>({
    ...
    defaultValues: {
      id: land.id,
      land_name: land.land_name || 'Test',
      land_code: land.land_code || 'Test',
      land_number: land.land_number || 'Test',
      size: land.size || 1,
      coordinations: coordination || 'test',
      location: land.location || 'Test',
      province: land.province || 'Test',
      district: land.district || 'Test',
      city: land.city || 'Test',
      owner: land.owner || 'Test',
      planttypeid: land.planttypeid || 1,
      categoryid: land.categoryid || 1,
      plant_date: land.plant_date || currentdate,
      harvest_cycle: land.harvest_cycle || 'Test',
      notes: land.notes || 'Test',
      created: land.created || formattedDate,
      createdby: land.createdby || 'Test',
      updated: land.updated || formattedDate,
      updatedby: land.updatedby || 'Test',
    },
  });
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="GEO Coordinations" {...field} disabled />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
